...
int main() {
    ...
    const int N = 10;  // 10k runs
    std::cout << "Benchmarking " << N << " runs (average):\n";
    
    double p1_avg = benchmark(N, [&]() { sum_invalid_ids(merged, is_id_repeated_twice); });
    double p2_avg = benchmark(N, [&]() { sum_invalid_ids(merged, is_id_repeated_Nth); });
    
    std::cout << "Part 1: " << sum_invalid_ids(merged, is_id_repeated_twice) << " (" << p1_avg << " ms)\n";
    std::cout << "Part 2: " << sum_invalid_ids(merged, is_id_repeated_Nth) << " (" << p2_avg << " ms)\n";
}